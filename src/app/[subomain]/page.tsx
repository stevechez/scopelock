...
export default async function PublicSubdomainPage({
	...
	// FIX 1: Next.js 15 requires params to be a Promise
	params: Promise<{ subdomain: string }>;
}) {
	// FIX 2: Unwrap the promise
	const { subdomain } = await params;

	const supabase = await createClient();

	// 1. FETCH THE TENANT DATA BASED ON THE SLUG
	const { data: tenant, error } = await supabase
		.from('tenants')
		.select('*')
		// FIX 3: Search the 'slug' column, not 'subdomain'!
		.eq('slug', subdomain)
		.single();

	console.log('DATABASE SEARCH FOR:', subdomain);
	console.log('DATABASE RESULT:', tenant);
	console.log('DATABASE ERROR:', error);

	// If no builder exists with this slug, show a 404
	if (error || !tenant) {
		notFound();
	}
