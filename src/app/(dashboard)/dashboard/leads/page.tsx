...
export default async function LeadCRMPage() {
	...
	return (
		<div className="min-h-screen bg-[#F8FAFC] pb-32">
			{/* Header */}
			<header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-border/60 px-6 py-4 flex justify-between items-center shadow-sm">
			...
			<main className="max-w-2xl mx-auto px-6 pt-10 space-y-8">
				...
				{!leads || leads.length === 0 ? (
					<div className="bg-white border-2 border-dashed border-border rounded-[2.5rem] p-12 text-center flex flex-col items-center">
						<div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4">
							<Zap size={24} className="text-slate-300" />
						</div>
						<h3 className="text-lg font-black text-foreground">Inbox Zero</h3>
					...
					<div className="space-y-6">
						{leads.map(lead => (
							<div
								key={lead.id}
								className="bg-white border border-border rounded-[2rem] p-6 shadow-sm"
							>
								{/* Lead Header */}
								...
								<div className="grid grid-cols-2 gap-3 mb-8">
									<div className="bg-slate-50 rounded-xl p-3 border border-border">
										<Hammer size={14} className="text-muted mb-2" />
									...
									<div className="bg-slate-50 rounded-xl p-3 border border-border">
										<DollarSign size={14} className="text-muted mb-2" />
										<p className="text-[10px] font-bold text-muted uppercase tracking-widest mb-0.5">
											Budget
										</p>
										<p className="text-sm font-black text-foreground">
									...
									<div className="bg-slate-50 rounded-xl p-3 border border-border col-span-2">
										<Clock size={14} className="text-muted mb-2" />
										<p className="text-[10px] font-bold text-muted uppercase tracking-widest mb-0.5">
											Timeline
										</p>
										<p className="text-sm font-black text-foreground">
								...
								<div className="flex gap-3">
									<button className="flex-1 bg-slate-50 text-muted font-bold py-4 rounded-2xl hover:bg-slate-100 transition-colors text-sm">
										Archive
									</button>
