...
// 📍 FIX: Add 'variantId: number' to the function signature
export async function getSubscriptionCheckoutURL(variantId: number) {
  try {
    // 1. Safety Check
    if (!variantId) {
      throw new Error("Please provide the required parameter: variantId.");
    }

    // ... Your existing Supabase user/tenant lookup ...

    // 2. The Lemon Squeezy API Call
    // Destructure 'data' and 'error' directly from the response
    ...
    // 3. Handle SDK Errors first
    if (error) {
      console.error("Lemon Squeezy SDK Error:", error);
      throw new Error("Payment provider rejected the request.");
    }

    // 4. Verify the nested JSON:API data structure
    // Note the double `data.data` – this satisfies the JSON:API spec
    if (!data || !data.data || !data.data.attributes) {
      throw new Error("Received malformed payload from Lemon Squeezy.");
    }

    // 5. Return the URL safely
