...
export default function OnboardingConcierge() {
	...
	return (
		<div className="bg-background text-foreground rounded-3xl border border-border border-border overflow-hidden shadow-sm">
			{/* Header / Progress Bar */}
			<div className="p-8 border-b border-border border-border bg-slate-50/50 dark:bg-slate-800/30">
				<div className="flex items-center justify-between mb-6">
					<div className="flex items-center gap-3">
						...
						<div>
							<h2 className="text-xl font-black text-foreground text-foreground">
								Quick Start Guide
							</h2>
						...
						>
							...
							<div className="flex-1">
								<div className="flex items-center justify-between mb-1">
									<h3
										className={`font-bold transition-all ${isDone ? 'text-muted line-through' : 'text-foreground text-foreground'}`}
									>
										{milestone.title}
									</h3>
									<span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-muted bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
										<Clock className="w-3 h-3" /> {milestone.estimatedTime}
									</span>
								</div>
								<p
									className={`text-sm mb-4 ${isDone ? 'text-muted' : 'text-muted'}`}
								>
